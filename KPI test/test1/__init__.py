    ...
    for i in range(0,traceID_num-1):
        ...
        for z in range(0,cols_value_num -1):
            if cols_value[z] == traceID[i]:
                previous_list = table.col_values(ncols -2)
                previous = float(previous_list[z])
                compare_result = round((average - previous)/previous,2)
                if compare_result >= 0.3 or compare_result <= -0.3:
                    wb = copy(excel_path)
                    ws = wb.get_sheet(0)
                    ws.write(z,ncols-1,average,set_style('Arial',220,True))
                #wb.save()
                else:
                    pass
            else:
...
#can not be use now!
def compare_result():
    origin_excel = xlrd.open_workbook(r'D:\\work\\KPI\\performance\\funrom performace1010_new (2).xls')
    sheet = origin_excel.sheet_by_name('Sheet1')
    rows = sheet.nrows
    cols = sheet.ncols
    copy_excel = copy(origin_excel)
    copy_s = copy_excel.get_sheet(0)
    for w in range(0,rows-1):
        last = copy_s.write
        previous = sheet.col_values(cols-2) 
        compare_result = round((float(last) - float(previous))/previous,2)
        if compare_result >= 0.3 or compare_result <= -0.3:
            set_style('Arial',220,True)
        else:
            pass
    copy_excel.save('D:\\work\\KPI\\performance\\funrom performace1010_new (2).xls')
